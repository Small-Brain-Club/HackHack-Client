    ...
    public void onUpdate()
    {
        if (!this.mc.player.capabilities.isCreativeMode && !this.mc.isIntegratedServerRunning())
        {
            ...
            } else
            {
                ...
                {
                    entityIter = this.mc.world.loadedTileEntityList.iterator();
                ...
                if (this.getSettings().get(6).toToggle().state)
                {
                    ...
                    while (entityIter.hasNext())
                    {
                        ...
                        if (!e.getDisplayName().equals(this.mc.player.getDisplayName()) && (double) this.mc.player.getDistance(e) <= this.getSettings().get(7).toSlider().getValue())
                        {
                            this.logOut("Logged Out " + this.mc.player.getDistance(e) + " Blocks Away From A Player (" + e.getDisplayName() + ")");
                        }
                    }
                }
